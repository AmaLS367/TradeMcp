  ...
  const guides: Record<string, unknown> = {
    overview: {
      ...
      userIntentToTool: {
        ...
        'OHLCV / candles / technical analysis': 'get_taapi_bulk_indicators for ready crypto indicators, get_binance_klines for raw candles',
        ...
        'technical indicator (SMA/RSI/MACD)': 'get_technical_indicator_catalog first, then get_taapi_indicator/get_taapi_bulk_indicators for crypto or get_technical_indicator for forex',
    ...
    technical_crypto: {
      ...
      workflow: [
        ...
        'Use get_taapi_bulk_indicators for ready crypto indicators when TAAPI.IO is connected; use get_taapi_indicator for one-off or uncommon TAAPI indicators.',
        'Fetch candles for the requested timeframe and one higher timeframe with get_binance_klines when raw OHLCV or a TAAPI fallback is needed. If no timeframe is specified, use 1h and 1d as defaults.',
