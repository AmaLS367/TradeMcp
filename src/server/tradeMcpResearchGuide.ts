  ...
  const guides: Record<string, unknown> = {
    overview: {
      ...
      userIntentToTool: {
        ...
        'OHLCV / candles / technical analysis': 'calculate_indicators for crypto RSI/MACD, get_binance_klines for raw candles',
        ...
        'technical indicator (SMA/RSI/MACD)': 'get_technical_indicator_catalog first, then calculate_indicators for crypto RSI/MACD or get_technical_indicator for forex',
    ...
    symbol_normalization: {
      ...
      providerFormats: {
        ...
        binance: { format: 'BASE/QUOTE', examples: ['BTC/USDT', 'ETH/USDT', 'SOL/USDT'], tools: ['get_binance_ticker', 'get_binance_order_book', 'get_binance_klines', 'get_binance_24h_stats', 'calculate_indicators', 'call_exchange_method'] },
        taapi: { format: 'BASE/QUOTE or compact BASEQUOTE', examples: ['AAVE/USDT', 'AAVEUSDT', 'BTC/USDT'], tools: ['get_taapi_indicator', 'get_taapi_bulk_indicators'] },
    ...
    technical_crypto: {
      ...
      workflow: [
        ...
        'Use calculate_indicators for crypto RSI and MACD. It fetches Binance candles and calculates indicators locally without a paid provider.',
        'Fetch candles for the requested timeframe and one higher timeframe with get_binance_klines when raw OHLCV or a not-yet-local indicator is needed. If no timeframe is specified, use 1h and 1d as defaults.',
