...
- [x] Add a dedicated "Connect AI Clients" dashboard section for ChatGPT, Claude, Gemini CLI, Cursor, and other MCP clients.
- [x] Provide copy-ready client configuration snippets for each supported AI client.
- [ ] Add client-specific setup checks so users can confirm OAuth and MCP discovery work before using an agent.
- [x] Add optional tool profiles such as `safe_research`, `trading_review`, and `full_access`.
- [x] Add `search` and `fetch` tools for better compatibility with AI clients that expect document-style connectors.
...
- [x] Split raw exchange access away from normal safe tools.
...
## Agent Experience
- [x] Improve tool descriptions continuously based on real agent mistakes.
- [x] Add short examples for common tool calls.
- [x] Add structured error messages that tell the agent exactly what the user needs to configure.
- [x] Add canonical symbol normalization guidance for CoinGecko IDs, Binance symbols, and FX pairs.
- [x] Add a "recommended tool" guide for common user intents.
...
## Observability
- [x] Log MCP tool calls with user id, client type, tool name, latency, and result status.
- [x] Detect client type from headers where possible.
- [x] Add dashboard metrics for tool usage and failures.
- [ ] Track real-time order execution events and delivery status for user notifications.
- [x] Add upstream provider latency tracking.
- [x] Add alerts for repeated provider failures or auth problems.

## Testing And Quality
- [ ] Add integration tests for proposal approve/reject Firestore rule compatibility.
- [x] Add tests for MCP tool list generation with enabled marketplace servers.
...
## Documentation
- [x] Document each MCP tool group and when agents should use it.
- [x] Add setup pages for ChatGPT, Claude, Gemini CLI, and Cursor.
- [x] Document the difference between Data Providers, Exchange Connections, and MCP Market.
- [x] Add troubleshooting docs for OAuth, Firebase rules, provider keys, and upstream MCP failures.
- [x] Keep README concise and move detailed guides into separate docs.
